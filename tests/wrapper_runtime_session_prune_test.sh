...
touch -d '10 minutes ago' "${stale_session}"
touch -d '1 minute ago' "${fresh_session}"
...
if [ -d "${stale_session}" ]; then
    echo "Expected stale runtime session to be pruned" >&2
    find "${runtime_parent}" -maxdepth 1 -mindepth 1 -type d | sort >&2
    exit 1
fi

if [ ! -d "${fresh_session}" ]; then
    echo "Expected fresh runtime session to be preserved" >&2
    find "${runtime_parent}" -maxdepth 1 -mindepth 1 -type d | sort >&2
    exit 1
fi

echo "PASS: runtime session pruning removes stale sessions older than 5 minutes"
